...
(function ($, window, document, undefined) {
    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            // TODO: _openEditModal 控制 edit 表單的樣子，搭配table中的數值
            /**
            ...
            _openEditModal: function () {
                ...
                let columnDefs = [];
                // only select the column name is not empty and  null
                for (let i in this.completeColumnDefs()){
                    if (this.completeColumnDefs()[i].name !== null && this.completeColumnDefs()[i].name !== ""){
                        columnDefs.push(this.completeColumnDefs()[i]);
                    }
                }


                let data = this.createDialog(columnDefs, this.language.edit.title, this.language.edit.button,
                    this.language.modalClose, 'editRowBtn', 'altEditor-edit-form');


                let selector = this.modal_selector;
                let rowData =  adata.data()[0]; // {"reserveId": 1, "jobId": "1", ....}

                // 將每一個 column的數值填入 form 表單中
                for (let obj of columnDefs) {
                    if (obj.name != null) {

                        let jquerySelector = "#" + obj.name.toString().replace(/\./g, "\\.");

                        let arrIndex = obj.name.toString().split(".");
                        let selectedValue;



                        // selectedValue: 此column的數值
                        for (let index = 0; index < arrIndex.length; index++) {
                            if (rowData) {
                                selectedValue = rowData[arrIndex[index]];
                            }
                        }
                        ...
                        // Added Select2
                        // if (obj.type.indexOf("select") >= 0 && obj.select2) {
                        ...
                        if (obj.type.indexOf("select") >= 0){
                            ...
                            for (let option of obj.options){
                                if (selectedValue === option.label) {
                                    $(selector).find(jquerySelector)[0];
                                    break;
                                }
                            }
                        ...
                        // Added checkbox
                        else if (obj.type.indexOf("checkbox") >= 0) {
                        ...
                        // Added date
                        else if (obj.type.indexOf("date") >= 0) {
                            if (obj.dateFormat !== "") {
                                let mDate = moment(this._quoteattr(selectedValue));
                                if (mDate && mDate.isValid()) {
                                    $(selector).find(jquerySelector).val(mDate.format(obj.dateFormat));
                                }
                            }
                        }
            ...
            _openDeleteModal: function () {
                ...
                var fill = function () {
                    ...
                    $(selector).find('.modal-body').html(that.language.deleteMessage || `<h5>Are you sure you wish to delete this data?</h5>`);
            ...
            */
            // TODO: createDialog 控制表單的樣子，不包含已選擇的數值
                        ...
                        if (columnDefs[j].type.indexOf("select") >= 0) {
                            ...
                            if (optionsArray.length > 0) {
                                ...
                                for (var i = 0; i < optionsArray.length; i++) {
                                    options += "<option value='" + optionsArray[i].value
                                        + "'>" + optionsArray[i].label + "</option>";
                                }
                            } else {
            ...
            _addRowData: function () {
                ...
                // 將 rowDataArray (表單中的資料) 更新至原table
                var checkFilesQueued = function() {
                    if (numFilesQueued == 0) {
                        that.onAddRow(that,
                            rowDataArray,
            ...
            _editRowCallback: function (response, status, more) {
                    ...
                    var data = (typeof response === "string") ? JSON.parse(response) : response;
                    console.log(data);
                    console.log(this.s.dt.row({selected: true}));
                    var selector = this.modal_selector;
                    $(selector + ' .modal-body .alert').remove();
